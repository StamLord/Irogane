...
var coin_prefab = preload("res://prefabs/coin_2_prefab.tscn")
...
func throw_coin_2():
	...
	coin.name = "CoinPrefab"	# If we don't set the name, 
								# after the first one the rest will be named @Rigidbody#
	coin.global_position = global_position - main_camera.global_basis.z * 1.2
	coin.look_at(main_camera.global_position)
	
	coin.start_force(owner_velocity + -main_camera.global_basis.z * 10.0)
